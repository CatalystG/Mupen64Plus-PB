...
void (*keyUpDown)(void *event) = dummyinput_Native_KeyUpDown;
...
static m64p_error plugin_connect_input(m64p_dynlib_handle plugin_handle)
{
    ...
    if (plugin_handle == NULL)
    {
        ...
        keyUpDown = dummyinput_Native_KeyUpDown;
        l_InputAttached = 0;
    }
    else
    {
        ...
        keyUpDown = (ptr_Native_KeyUpDown) osal_dynlib_getproc(plugin_handle, "Native_KeyUpDown");
        if (getVersion == NULL || controllerCommand == NULL || getKeys == NULL || initiateControllers == NULL ||
            readController == NULL || romOpen_input == NULL || romClosed_input == NULL || keyDown == NULL || keyUp == NULL || keyUpDown == NULL)
        {
...
static m64p_error plugin_connect_gfx(m64p_dynlib_handle plugin_handle)
{
    ...
    else
    {
        ...
        readScreen = (ptr_ReadScreen2) osal_dynlib_getproc(plugin_handle, "ReadScreen");
...
static m64p_error plugin_start_input(void)
{
    ...
    DebugMessage(M64MSG_WARNING, "Core Controller Raw:%d\n", Controls[0].RawData);
