...
m64p_error ConfigInit(const char *ConfigDirOverride, const char *DataDirOverride)
{
    ...
    {
    	DebugMessage(M64MSG_INFO, "Opened configuration file '%s'.", filepath);
    }
    free(filepath);
...
EXPORT m64p_error CALL ConfigSaveFile(void)
{
    ...
    if(l_ConfigDirOverride != NULL)
    {
    	configpath = l_ConfigDirOverride;
    }
    else
    {
    	configpath = ConfigGetUserConfigPath();
    ...
    DebugMessage(M64MSG_INFO, "Trying to save configuration file '%s'.", filepath);
    if (fPtr == NULL)
    {
