...
#define OPENGL_DEBUG_H
  ...
  #define OPENGL_CHECK_ERRORS { const GLenum errcode = glGetError(); if (errcode != GL_NO_ERROR) {fprintf(stdout, "OpenGL Error code %i in '%s' line %i\n", errcode, __FILE__, __LINE__-1);fflush(stdout);} }
#else
